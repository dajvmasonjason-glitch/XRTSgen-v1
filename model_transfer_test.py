...
# 数据目录设置
train_data_dir = r"E:\data_various_width\dft"
...
if __name__ == "__main__":
    # 模型参数设置
    lr = 0.0001  # 微调学习率，通常比从头训练小
    wd = 1e-5
    block_type = 'block300-frozen41'
    dropout = 0.2
    epochs = 4000
    max_grad_norm = 1.0
    ...
    # 预训练模型路径
    pretrained_model_path = "model/best_model_rpa_train_block300_lr0.001_dpt0.2_wd1e-05_epoch262.pth"  # 修改为实际路径
    ...
    # 五折交叉验证
    for fold, (train_ids, val_ids) in enumerate(kfold.split(labels_train)):
        print(f"\n===== 开始第 {fold+1}/{k_folds} 折交叉验证 =====")
        
        # 每次都重新加载预训练模型，避免折叠间的影响
        model, pretrained_stats = load_pretrained_model(pretrained_model_path, freeze_features=freeze_features)
        
        # 从原始训练集中划分当前折叠的训练集和验证集
        y_train_fold, labels_train_fold = y_train[train_ids], labels_train[train_ids]
        y_val_fold, labels_val_fold = y_train[val_ids], labels_train[val_ids]
        
        # 使用预训练模型的统计信息初始化数据集
        train_dataset = InverseDataset(labels_train_fold, y_train_fold, 
                                     (pretrained_stats['label_mean'], pretrained_stats['label_std']),
                                     (pretrained_stats['y_mean'], pretrained_stats['y_std']))
        val_dataset = InverseDataset(labels_val_fold, y_val_fold, 
                                   (pretrained_stats['label_mean'], pretrained_stats['label_std']),
                                   (pretrained_stats['y_mean'], pretrained_stats['y_std']))
        ...
        for epoch in range(epochs):
            ...
            val_r2_batch /= len(val_loader)
            
            # 记录损失和R²
            train_losses.append(train_loss)
            train_r2.append(train_r2_batch)
            val_losses.append(val_loss)
            val_r2.append(val_r2_batch)
            ...
            f"Val Loss: {val_loss:.6f}, Val R平方: {val_r2_batch:.6f}")
            ...
            if val_loss < best_val_loss:
                ...
                if counter >= early_stopping_patience:
                    print(f"早停触发！验证损失在 {early_stopping_patience} 轮内没有改善。")
                    print(f"最佳epoch: {best_epoch}, 最佳验证损失: {best_val_loss:.6f}")
                    break
        
        # 创建统一的输出文件
        file_prefix = f"tl_fold{fold+1}_{block_type}_lr{lr}_dpt{dropout}_wd{wd}_epoch{best_epoch}"
        output_file_path = f"results_{file_prefix}.txt"
        
        with open(output_file_path, 'w') as output_file:
            # 写入交叉验证信息
            output_file.write(f"===== 第 {fold+1}/{k_folds} 折交叉验证结果 =====\n\n")
            
            # 写入训练参数
            output_file.write("训练参数:\n")
            output_file.write(f"学习率: {lr}\n")
            output_file.write(f"权重衰减: {wd}\n")
            output_file.write(f"Dropout: {dropout}\n")
            output_file.write(f"最佳Epoch: {best_epoch}\n")
            output_file.write(f"最佳验证损失: {best_val_loss:.6f}\n\n")
            
            # 写入训练损失数据
            output_file.write("\n===== 训练损失数据 =====\n")
            output_file.write('Epoch,Train Loss,Val Loss\n')
            for epoch in range(len(train_losses)):
                output_file.write(f'{epoch+1},{train_losses[epoch]},{val_losses[epoch]}\n')
            
            # 写入R²数据
            output_file.write("\n===== R平方数据 =====\n")
            output_file.write('Epoch,Train_R平方,Val_R平方\n')
            for epoch in range(len(train_r2)):
                output_file.write(f'{epoch+1},{train_r2[epoch]},{val_r2[epoch]}\n')
            
            # 写入等高线评估结果
            output_file.write("\n===== 等高线评估结果 =====\n")
            
            # 使用验证集进行等高线评估，传入output_file
            accuracy, max_ratio, ratio_dist = evaluate_dataset(model, labels_val_fold, y_val_fold, x_coords, "val", file_prefix, pretrained_stats, output_file)
            
        print(f"\n所有结果已保存到 {output_file_path}")
